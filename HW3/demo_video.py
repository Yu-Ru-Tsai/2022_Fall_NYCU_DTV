...
from track import eval_seq, eval_webcam
import cv2
import time
...
def track_webcam(opt):  
    ...
    frame_dir = None if opt.output_format=='text' else osp.join(result_root, 'frame') 
    try: 
        eval_webcam(opt, 'mot', result_filename, save_dir=frame_dir, show_image=True, frame_rate=frame_rate)
    except Exception as e:
        logger.info(e)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='weight/jde.1088x608.uncertainty.pt', help='path to weights file')
    ...
    parser.add_argument('--input-video', type=str, default='demo_video/demo_video.mp4', help='path to the input video')
    parser.add_argument('--output-format', type=str, default='video', choices=['video', 'text'], help='Expected output format. Video or text.')
    parser.add_argument('--output-root', type=str, default='output', help='expected output root path')
