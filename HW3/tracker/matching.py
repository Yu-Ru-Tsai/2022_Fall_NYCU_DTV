...
def iou_distance(atracks, btracks):
    ...
    _ious = ious(atlbrs, btlbrs) ## shape: (len(a_tlbrs), len(b_tlbrs))
    cost_matrix = 1 - _ious

    return cost_matrix

def embedding_distance(tracks, detections, metric='cosine'): ## cosine similarity
    """
...
def fuse_motion(kf, cost_matrix, tracks, detections, only_position=False, lambda_=0.98):
    if cost_matrix.size == 0:
        return cost_matrix
    gating_dim = 2 if only_position else 4 ## usually 4
